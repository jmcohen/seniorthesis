"""
NOTE: THIS CODE IS FROM NILEARN (but modified slightly)
...
def mfista(f1_grad, f2_prox, total_energy, lipschitz_constant, w_size,
    ...
    for i in range(max_iter):
        ...
        for _ in range(10):
            ...
            if ista_step and old_energy <= energy:
...
def mfista_backtrack(f1, f2, f1_grad, f2_prox, lipschitz_constant, w_size, max_iter=1000, tol=1e-4,
           check_lipschitz=False, callback=None,verbose=2):
    """Generic FISTA solver.
    ...
    total_energy = lambda x : f1(x) + f2(x)
    ...
    for i in range(max_iter):
        ...
        # invoke callback
        if verbose:
            print('mFISTA: Iteration % 2i/%2i: E = %7.4e, dE % 4.4e' % (i + 1, max_iter, old_energy, energy_delta))
        if callback and callback(locals()):
            break
        if np.abs(energy_delta) < tol:
            if verbose:
        ...
        for j in range(100):
            w = f2_prox(z - (1/L) * gradient_buffer, (1/L))
            # print t, norm(w), L, total_energy(w), f1(z) + np.dot(w - z, gradient_buffer) + (L/2)*norm(w - z)**2 + f2(w)
            if total_energy(w) <= f1(z) + np.dot(w - z, gradient_buffer) + (L/2)*norm(w - z)**2 + f2(w):
                break
            L *= 2

        print "\t%d backtrack steps" % j
        energy = total_energy(w)
...
def mfista_project(f1_grad, f2_prox, total_energy, lipschitz_constant, w_size, project,
           dgap_tol=None, init=None, max_iter=1000, tol=1e-4,
           check_lipschitz=False, dgap_factor=None, callback=None,
           verbose=2):
    """Generic FISTA solver.
